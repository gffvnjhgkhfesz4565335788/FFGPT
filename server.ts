...
const authorizer = process.env.OAUTH_AUTHORIZER_URL || "https://dev-12x03gsex3m5hkfa.us.auth0.com/";
const issuer = authorizer.endsWith('/') ? authorizer : `${authorizer}/`;
...
async function getAuth0User(token: string) {
  const audience = process.env.OAUTH_RESOURCE || appUrl;
  
  try {
    // 1. Verify JWT Signature, Audience, and Issuer
    const { payload } = await jose.jwtVerify(token, JWKS, {
      issuer,
      audience,
    });
    
    // If the token has email claim, use it, otherwise fetch from /userinfo
    if (payload.email) {
       return payload;
    }

    // 2. Fallback to /userinfo if email is not in the JWT claims
      ...
      return {
        ...
        _meta: {
          "mcp/www_authenticate": [
            `Bearer resource_metadata="${appUrl}/.well-known/oauth-protected-resource", scope="project.write openid email profile offline_access", error="insufficient_scope", error_description="You need to login to continue"`
          ]
        }
      };
    }
...
async function startServer() {
  ...
  app.get(["/.well-known/oauth-protected-resource", "/.well-known/oauth-protected-resource/*"], (req, res) => {
    ...
    res.json({
      resource: process.env.OAUTH_RESOURCE || appUrl,
      authorization_servers: [authorizer],
      scopes_supported: ["project.write", "openid", "email", "profile", "offline_access"],
